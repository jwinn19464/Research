...
def set_atommap(mol, num=0):
    if mol is None:
        print("set_atommap: Received NoneType molecule.")
        return
    for atom in mol.GetAtoms():
...
def get_clique_mol(mol, atoms):
    try:
        # Attempt to generate SMILES with kekuleSmiles=True
        smiles = Chem.MolFragmentToSmiles(mol, atoms, kekuleSmiles=True)
    except KekulizeException:
        # Handle KekulizeException by trying with kekuleSmiles=False
        print("KekulizeException caught, retrying with kekuleSmiles=False")
        smiles = Chem.MolFragmentToSmiles(mol, atoms, kekuleSmiles=False)
    
    # Check if the SMILES conversion resulted in None or empty string
    if not smiles:
        raise ValueError("Failed to generate valid SMILES string.")
    
    new_mol = Chem.MolFromSmiles(smiles, sanitize=False)
    
    if new_mol is None:
        raise ValueError(f"Failed to convert SMILES to molecule: {smiles}")
    
    # Apply further processing to the molecule
